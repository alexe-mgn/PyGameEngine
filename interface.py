...
class Division:
    ...
    def post_init(self):
        """
        Переопределяемый.
        Вызывается сразу после инициализации
        (
        САМОГО ВЕРХНЕГО УРОВНЯ!!! Если вы переопределяете в post_init что-либо,
        то изменения могут быть отменены инициализацией подклассов, поскольку порядок вызова следующий:
        Division.__init__() -> post_init() -> subclass.__init__() -> ... -> self.__init__()
        )
        или активации (activate)
        """
        pass
